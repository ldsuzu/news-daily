...
用 `deepseek-flash`，**空闲时段**（夜间、周末、节假日，以及工作日 9-12、14-18 点之外）
单价 ¥1/百万 输入、¥4/百万 输出，缓存命中的输入只要 ¥0.02/百万。

实测单条消耗：**输入 81 token · 输出 20 token · ¥0.00016**。

| | 每天（约 350 条） | 每月 |
|---|---|---|
| **花费** | **约 ¥0.06** | **约 ¥1.7** |

定时任务跑在早上 7:00，正好在空闲时段，价格只有高峰的一半。
设置页和今日屏页脚实时显示今日 / 累计花费；`llm.daily_budget_cny` 是硬上限（默认 0.5），
当天花超了就自动停手。

### 省 token 的五个手段

1. **中文条目不让模型再写一遍中文** —— 只有外文标题才需要翻译和摘要。
   这一条砍掉了约 70% 的输出 token（输出单价是输入的 4 倍，所以省得最多）。
2. **批量送**：一次 20 条共享一份系统提示，还能吃上缓存折扣。
3. **关掉思考模式**（`thinking: disabled`）：翻译和分类是机械活儿，不需要推理链。
   开着的话 reasoning token 按输出计费，而且长任务会因为思考没写完就撞上 token 上限、
   返回**空内容** —— 事件归并一开始就是因此静默失败的。
4. **只处理新条目**：`llm_at` 为空才排队，重跑不重复花钱。
5. **单轮上限 + 单日预算**：`max_items_per_run` 与 `daily_budget_cny` 兜底。

### 内容筛选

游戏和科技媒体会发大量泛生活新闻 —— 游民星空发过「500 元以上月饼礼盒销量暴跌」
「苹果 Beats 全断耳机」，Solidot 发过「6 岁女孩打破魔方世界纪录」。

所以每条资讯都会被判一个 `rel`（是否属于科技 / 游戏 / AI 行业），
**不相关的进不了日报和精选**，但仍然留在库里、仍然搜得到。实测一天 211 条入库、约 45 条被剔除。

### 不想花钱

`llm.enabled: false` 时其它功能完全不受影响 —— 只是没有翻译和热度分，
排序退回规则分（源权重 + 时效 + 关键词）。
